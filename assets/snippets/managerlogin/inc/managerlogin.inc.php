<?php
//2015/10/05 - EPO -> ajout de genHash pour faire le calcul du hash nouvelle version ModX
//
...
//		echo 'username :<pre>'.print_r($username, true).'</pre>';
        ...
        switch($action) {
            ...
            case 'login':
                ...
                $modx->invokeEvent("OnBeforeManagerLogin", array("username"=>$username,
                ...
                $rt = $modx->invokeEvent("OnManagerAuthentication", array("userid"=>$usr_internalKey,
                ...
                if (!$rt||(is_array($rt) && !in_array(true,$rt))) {
                    ...
                    if (!$errors) {
//echo "$usr_password != genHash($form_password,$usr_internalKey)".genHash($form_password,$usr_internalKey);
                        //if ($usr_password != md5($form_password)) {
                        if ($usr_password != genHash($form_password,$usr_internalKey)) {
                ...
                if ($errors) {
//echo "$usr_password != md5($form_password)):".md5($form_password); print_r($errors);exit;
                    session_destroy();
                    session_unset();
                } else {
        ...
        // Custom CSS ?
        if (!empty($cssStyle)) { $modx->regClientCSS($cssStyle); }
