...
class ContentVariable {
		...
		if ($tv) {
			...
			if ((!isset ($tv['id']) || !$tv['id']) && $name) {
				...
				switch ($name) {
					...
					case 'pub_date' :
						$type = 'date';
						$caption = $QE_lang['document_pub_date'];
						$description = $QE_lang['document_pub_date_help'];
						$group = 'content';
						if ($value != 0) $value = strftime("%d-%m-%y %H:%M:%S", $value);
						break;
						
					case 'unpub_date' :
						$type = 'date';
						$caption = $QE_lang['document_unpub_date'];
						$description = $QE_lang['document_unpub_date_help'];
						$group = 'content';
						if ($value != 0) $value = strftime("%d-%m-%y %H:%M:%S", $value);
						break;
	...
	function checkPermissions($userId = 0) {
		...
		if (!$modx->config['use_udperms'] || $userId == 1) {
			...
			//if (ereg('^[0-9]+$', $varId)) { // EPO - 20120618- ereg obsolete
			if (preg_match('#^[0-9]+$#', $varId)) {
